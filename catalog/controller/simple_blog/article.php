...
//					$image = HTTP_SERVER . 'image/' . $result['featured_image'];
					$featured_found = 1;
					$image = $this->model_tool_image->resize($result['featured_image'], 500, 300);
...
//						$data['image'] = HTTP_SERVER . 'image/' . $article_info['featured_image'];
						$image = $this->model_tool_image->resize($article_info['featured_image'], 873, 585);
					} else if($article_info['image']) {
//						$data['image'] = HTTP_SERVER . 'image/' . $article_info['image'];
						$featured_found = '';
						$image = $this->model_tool_image->resize($article_info['image'], 873, 585);
					} else {
						$data['image'] = '';
						$featured_found = '';
					}
...
>>>>>>> 5b727248a58fafe0cd14b54a2ca2e114a8ed3dd5
                    ...
                    foreach($related_articles as $article) {
                        if($article['image']) {
                            $article_image = $this->model_tool_image->resize($article['image'], 230, 230);
                        } else if($article['image']) {
                            $article_image = $this->model_tool_image->resize($article['image'], 230, 230);
                        } else {
<<<<<<< HEAD
                            $article_image = $this->model_tool_image->resize('no_image.jpg', 230, 230);
=======
                            $article_image = $this->model_tool_image->resize('no_image.png', 230, 230);
>>>>>>> 5b727248a58fafe0cd14b54a2ca2e114a8ed3dd5
                        }
                ...
                if(isset($this->request->get['simple_blog_category_id'])) {
                    $data['breadcrumbs'][] = array(
                        'text' => $this->language->get('heading_title'),
                        'href' => $this->url->link('simple_blog/category', 'simple_blog_category_id=' . $this->request->get['simple_blog_category_id'], 'SSL')
                    );
                }

                $data['breadcrumbs'][] = array(
                    'text'      => $article_info['article_title'],
                    'href'      => $this->url->link('simple_blog/article/view', 'simple_blog_article_id=' . $article_info['simple_blog_article_id'])
                );
		...
		public function writeComment() {
			...
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				...
				if ((utf8_strlen($this->request->post['comment_name']) < 3) || (utf8_strlen($this->request->post['comment_name']) > 25)) {
					$json['error'] = $this->language->get('error_name');
				}
	
				if ((utf8_strlen($this->request->post['comment_text']) < 3) || (utf8_strlen($this->request->post['comment_text']) > 1000)) {
					$json['error'] = $this->language->get('error_text');
				}
	
				if (empty($this->session->data['captcha']) || ($this->session->data['captcha'] != $this->request->post['comment_captcha'])) {

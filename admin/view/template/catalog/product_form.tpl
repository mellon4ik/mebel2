...
<<<<<<< HEAD
=======
              <?php if (isset($product_id) && $proposition_status) { ?>
                <li><a href="#tab-profitable-proposition" data-toggle="tab">Выгодное предложение<?php //echo $profitable_proposition; ?></a></li>
              <?php } ?>
...
=======
              <?php if (isset($product_id) && $proposition_status) { ?>
              <div class="tab-pane" id="tab-profitable-proposition">
                  <div class="clearfix">
                      <blockquote>
                          <p><?php echo $text_warning; ?></p>
                      </blockquote>
                      <div class="col-sm-4">
                          <input type="text" class="form-control" id="proposition_name" placeholder="<?php echo $entry_proposition_name; ?>"/>
                      </div>
                      <button type="button" onclick="proposition.add();" data-toggle="tooltip" title="<?php echo $button_proposition_add; ?>" class="btn btn-primary pull-left"><i class="fa fa-plus-circle"></i></button>
                      <button class="btn btn-info pull-right proposition-save" onclick="proposition.save();" type="button" title="" data-toggle="tooltip" data-original-title="<?php echo $button_save; ?>"><i class="fa fa-save"></i></button>
                  </div>
                  <hr/>
                  <div class="table-responsive">
                      <?php $proposition_row = 0; ?>
                      <?php $product_row = 0; ?>
                      <?php foreach ($propositions as $proposition) { ?>
                          <table class="table table-bordered table-hover" id="proposition_<?php echo $proposition_row; ?>">
                              <caption><?php echo $proposition['name']; ?>
                                  <div class="pull-right">
                                      ...
                                      <button type="button" data-toggle="tooltip" onclick="proposition.remove('<?php echo $proposition_row; ?>');" title="<?php echo $button_remove; ?>" class="btn btn-danger pull-right"><i class="fa fa-minus-circle"></i></button>
                                  </div>
                              </caption>
                              <thead>
                              <tr>
                                  <td class="text-left"><?php echo $text_proposition_product_name; ?></td>
                                  <td class="text-left"><?php echo $text_proposition_price; ?></td>
                                  <td class="text-left"><?php echo $text_proposition_price_included; ?></td>
                                  <td class="text-left"><?php echo $text_proposition_amount; ?></td>
                                  <td class="text-left"><?php echo $text_proposition_sort; ?></td>
                                  <td></td>
                              </tr>
                              </thead>
                              <tbody>
...
<?php if (isset($product_id) && $proposition_status) { ?>
    <script>
        var proposition_row = <?php echo$proposition_row; ?>;
        var product_row = <?php echo $product_row; ?>;

        var proposition = {
            'add': function () {
                var proposition_name = $('#proposition_name').val();
                if (proposition_name.length > 0) {
                    var html = '<table class="table table-bordered table-hover" id="proposition_' + proposition_row + '">' +
                        '<caption>' + proposition_name +
                        '<div class="pull-right"><button class="btn btn-info proposition-save" onclick="proposition.save();" type="button" title="" data-toggle="tooltip" data-original-title="<?php echo $button_save; ?>"><i class="fa fa-save"></i></button> <button type="button" data-toggle="tooltip" onclick="proposition.remove(\'' + proposition_row + '\');" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></div>' +
                        '</caption>' +
                        '<thead>' +
                        '<tr>' +
                        '<td class="text-left"><?php echo $text_proposition_product_name; ?></td>' +
                        ...
                        '<td class="text-left"></td>' +
                        '</tr>' +
                        '<td colspan="3"><input id="proposition_product_add_' + proposition_row + '" data-proposition-row="' + proposition_row + '" class="form-control" type="text" placeholder="<?php echo $entry_proposition_product; ?>"/></td>' +
                        '<td colspan="3"></td>' +
                        '</tr>' +
                        '<tr>' +
                        '<td class="text-left"><?php echo $text_proposition_sum; ?></td>' +
                        ...
                        '</table>' +
                        '<div class="clearfix"></div>';
                    $('#tab-profitable-proposition .table-responsive').append(html);
                    productAutocomplete(proposition_row);
                    ++proposition_row;
                }
            },
            'remove': function (id) {
                $('table#proposition_' + id).remove();
            },
            'save': function () {
                var result = {};
                $('#tab-profitable-proposition .table-responsive table').each(function () {
                    var proposition_id = $(this).attr('id').split('_')[1];
                    var proposition_name = $.trim($(this).find('caption').text());
                    ...
                    $(this).find('tbody tr[data-product-id]').each(function (index, element) {
                        ...
                        var amount = $(element).find('td#amount input').val();
                        var sort = $(element).find('td#sort input').val();
                        result[proposition_id]['products'][index] = {};
                        result[proposition_id]['products'][index]['product_id'] = product_id;
                        result[proposition_id]['products'][index]['price_included'] = price_included;
                        result[proposition_id]['products'][index]['amount'] = amount;
                        result[proposition_id]['products'][index]['sort'] = sort;
                    });
                });


                $.ajax({
                    url: 'index.php?route=catalog/product/save_proposition&token=<?php echo $token; ?>&product_id=' + <?php echo $product_id; ?>,
                    type: 'POST',
                    data: result,
                    dataType: 'json',
                    success: function (json) {
            ...
            'product': {
                'remove': function (proposition_id, product_id) {
                    $('table#proposition_' + proposition_id + ' tr#product_' + product_id).remove();
                }
            }
        }

        function productAutocomplete(proposition_row) {
            $('input#proposition_product_add_' + proposition_row).autocomplete({
                'source': function (request, response) {
                    $.ajax({
                        url: 'index.php?route=catalog/product/autocomplete&token=<?php echo $token; ?>&filter_name=' + encodeURIComponent(request),
                        dataType: 'json',
                        success: function (json) {
                            response($.map(json, function (item) {
                                return {
                                    label: item['name'],
                                    value: item['product_id'] + ':' + item['price']
                                }
                            }));
                        }
                    });
                ...
                'select': function (item) {
                    var name = item['label'];
                    var price = item['value'].split(':')[1];
                    var id = item['value'].split(':')[0];

                    $('input#proposition_product_add').val('');

                    var proposition_id = $(this).attr('data-proposition-row');

                    var html = '<tr id="product_' + product_row + '" data-product-id="' + id + '">' +
                        '<td class="text-left" id="name">' + name + '</td>' +
                        '<td class="text-left" id="price">' + price + '</td>' +
                        '<td class="text-left" id="price_included"><input class="price_included form-control" type="text" value=""/></td>' +
                        '<td class="text-left" id="amount"><input class="form-control" type="text" value=""/></td>' +
                        '<td class="text-left" id="sort"><input class="form-control" type="text" value=""/></td>' +
                        '<td class="text-left">' +
                        '<button type="button" onclick="proposition.product.remove(\'' + proposition_id + '\', \'' + product_row + '\');" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button>' +
                        '</td>' +
                        '</tr>';
        ...
        for(var i = 0; true; ++i) {
            if($('#proposition_product_add_' + i).length > 0) {
        ...
        function generateTotal() {
            for (var i = 0; true; ++i) {
                if ($('#proposition_' + i).length > 0) {
                    var price_total = 0;
                    var price_included_total = 0;
                    $('#proposition_' + i + ' tr[data-product-id]').each(function (index, element) {
                        price_total += Number($(element).find('#price').text());
                        price_included_total += Number($(element).find('#price_included input').val());
                    });
                    $('#proposition_' + i + ' #price_total').text(price_total + <?php echo $price; ?>);
                    $('#proposition_' + i + ' #price_included_total').text(price_included_total + <?php echo $price; ?>);
                }
                else {
                    break;
                }

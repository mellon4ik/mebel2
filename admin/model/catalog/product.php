...
=======
    ...
    public function savePropositions($product_id, $data)
    {
        $query = $this->db->query("SELECT proposition_id FROM " . DB_PREFIX . "proposition WHERE product_id = " . (int)$product_id);
        if ($query->num_rows > 0) {
            foreach($query->rows as $proposition) {
                $proposition_id = $proposition['proposition_id'];
                $this->db->query("DELETE FROM " . DB_PREFIX . "proposition WHERE product_id = " . (int)$product_id);
                $this->db->query("DELETE FROM " . DB_PREFIX . "proposition_product WHERE proposition_id = " . (int)$proposition_id);
            }
        }

        foreach($data as $proposition) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "proposition SET product_id = " . (int) $product_id . ", `name` = '" . $this->db->escape($proposition['name']) . "'");

            $proposition_id = $this->db->getLastId();

            foreach($proposition['products'] as $product) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "proposition_product SET proposition_id = " . (int) $proposition_id . ", product_id = '" . (int) $product['product_id'] . "', price_included = '" . (double) $product['price_included'] . "', amount = '" . (int) $product['amount'] . "', sort = '" . (int) $product['sort'] . "'");
            }
        }
    }

    public function getPropositions($product_id) {
        $result = array();
        ...
        foreach($query_proposition->rows as $key => $proposition) {
            $result[$key]['name'] = $proposition['name'];
            ...
            foreach($query_products->rows as $key_product => $product) {
                ...
                $result[$key]['products'][$key_product]['price_included'] = $product['price_included'];
                $result[$key]['products'][$key_product]['amount'] = $product['amount'];
                $result[$key]['products'][$key_product]['sort'] = $product['sort'];
            }
        }
        return $result;
    }
